...
#[test]
fn tsc_import_helpers_namespace_recovers_awaited_return_value() {
    // Generated by TypeScript 5.9.3 with importHelpers and CommonJS output.
    // ES2015 retains a native generator; ES5 adds tslib.__generator as well.
    for input in [
        include_str!("fixtures/tslib-async/es2015/load.js"),
        include_str!("fixtures/tslib-async/es5/load.js"),
    ] {
        let output = render(input);
        assert!(output.contains("async function load(value)"), "{output}");
        assert!(output.contains("await value"), "{output}");
        assert!(output.contains("return result + 1"), "{output}");
        assert!(
